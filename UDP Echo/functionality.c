...
void MSGParser_About(char *buffer)
{
    ...
    write(message);
    sprintf(message, "IP Address: %d.%d.%d.%d\r\n",
           (uint8_t) (Glo.IP.host >> 24), (uint8_t) (Glo.IP.host >> 16),
           (uint8_t) (Glo.IP.host >> 8), (uint8_t) Glo.IP.host);
    write(message);
    sprintf(message, "Multicast Address: %d.%d.%d.%d\r\n",
           (uint8_t) (Glo.IP.multicast >> 24), (uint8_t) (Glo.IP.multicast >> 16),
           (uint8_t) (Glo.IP.multicast >> 8), (uint8_t) Glo.IP.multicast);
    write(message);
}
...
void MSGParser_Audio(char *buffer)
{
   bool transfer;
   int i;
   uint16_t outval[DISCOVER_COUNT];
   transfer = SPI_transfer(Glo.Devices.spi, &Glo.Devices.spiTransaction);
   if(!transfer)
   {    while(1);   }
   Glo.AudioOut = 0;
   for(i = 0; i < (Glo.Call.callcount + (int) Glo.Call.local); i++)
   {
       if(!Glo.AudioTx[i].state)
       {
           if(Glo.AudioTx[i].completedTx)
           {
               outval[i] = Glo.AudioTx[i].PingBuffTx[Glo.AudioTx[i].readindex];
               Glo.AudioTx[i].state = 1;
           }
           else
               outval[i] = AUDIO_INIT;
           return;
       }
       Glo.AudioTx[i].readindex++;
       if(Glo.AudioTx[i].readindex >= AUDIO_BLOCK_SIZE)
       {
           if(Glo.AudioTx[i].state == 1 && Glo.AudioTx[i].completedTx == Glo.AudioTx[i].PongBuffTx)
           {
               outval[i] = Glo.AudioTx[i].PongBuffTx[0];
               Glo.AudioTx[i].readindex = 0;
               Glo.AudioTx[i].state = 2;
           }
           else if(Glo.AudioTx[i].state == 2 && Glo.AudioTx[i].completedTx == Glo.AudioTx[i].PingBuffTx)
           {
               outval[i] = Glo.AudioTx[i].PingBuffTx[0];
               Glo.AudioTx[i].readindex = 0;
               Glo.AudioTx[i].state = 1;
           }
           else
       ...
       {
           if(Glo.AudioTx[i].state == 1)
           {    outval[i] = Glo.AudioTx[i].PingBuffTx[Glo.AudioTx[i].readindex];    }
           else
           {    outval[i] = Glo.AudioTx[i].PongBuffTx[Glo.AudioTx[i].readindex];    }
       }
       Glo.AudioOut += outval[i] / (Glo.Call.callcount + (int) Glo.Call.local);
    }
}
...
{
    char *loc, message[MessageLength], IPs[MessageLength];
    int i, j;
    uint32_t addr, addr2;
    loc = nextword(buffer);
    if(!*loc)
    {
        if(!Glo.Call.callcount)
        {
            write("No one is on the call.\r\n");
            return;
        }
        write("Addresses on call:\r\n");
        for(i = 0; i < Glo.Call.callcount; i++)
        {
            addr = Glo.Call.list[i];
            sprintf(message, "IP Address %d: %d.%d.%d.%d\r\n", i,
                   (uint8_t) (addr >> 24), (uint8_t) (addr >> 16),
                   (uint8_t) (addr >> 8), (uint8_t) addr);
            write(message);
        }
    }
    else if(isdigit(*loc) || *loc == 'd')
    {
        while(*loc)
        {
            if(isdigit(*loc))
            ...
            else
            {
                ...
                if(isdigit(*loc))
                    i = atoi(loc);
                if(i >= 0 && i < Glo.IP.addrcount)
                    addr = Glo.IP.discover[i];
                else
                {
                    write("Invalid Discovery Table entry!\r\n");
                    loc = nextword(loc);
                    continue;
                }
            }
        ...
        for(i = 0; i < Glo.Call.callcount; i++)
        {
            IPs[0] = 0;
            addr2 = Glo.IP.host;
            sprintf(message," %d.%d.%d.%d", (uint8_t) (addr2 >> 24), (uint8_t) (addr2 >> 16),
                   (uint8_t) (addr2 >> 8), (uint8_t) addr2);
            strcat(IPs, message);
            addr = Glo.Call.list[i];
            for(j = 0; j < Glo.Call.callcount; j++)
            {
                addr2 = Glo.Call.list[j];
                if(addr != addr2)
                {
                    sprintf(message," %d.%d.%d.%d", (uint8_t) (addr2 >> 24), (uint8_t) (addr2 >> 16),
                           (uint8_t) (addr2 >> 8), (uint8_t) addr2);
            ...
            sprintf(message, "-netudp %d.%d.%d.%d -dial connect%s", (uint8_t) (addr >> 24),
                    (uint8_t) (addr >> 16), (uint8_t) (addr >> 8), (uint8_t) addr, IPs);
            addPayload(message);
        }
    }
    else if(strstr(loc, "connect") == loc)
    {
        loc = nextword(loc);
        while(*loc)
        {
    ...
    else if(strstr(loc, "end") == loc)
    {
        loc = nextword(loc);
        if(!*loc)
        {
            addr2 = Glo.IP.host;
            j = Glo.Call.callcount;
            for(i = 0; i < j; i++)
            {
                addr = Glo.Call.list[i];
                sprintf(message, "-netudp %d.%d.%d.%d -dial end %d.%d.%d.%d",
                        (uint8_t) (addr >> 24),(uint8_t) (addr >> 16), (uint8_t) (addr >> 8),
                        (uint8_t) addr, (uint8_t) (addr2 >> 24), (uint8_t) (addr2 >> 16),
                        (uint8_t) (addr2 >> 8), (uint8_t) addr2);
                addPayload(message);
                removeCall(addr);
            }
            ...
            else if(*loc == 'c')
            {
                ...
                if(i >= 0 && i < Glo.Call.callcount)
                {
                    ...
                    sprintf(message, "-netudp %d.%d.%d.%d -dial end %d.%d.%d.%d",
                            ...
                            (uint8_t) (addr2 >> 8), (uint8_t) addr2);
                    addPayload(message);
                    removeCall(Glo.Call.list[i]);
                }
                else
                {
                    write("Invalid Call List index!\r\n");
                    loc = nextword(loc);
                    continue;
                }
            }
...
void MSGParser_Discover(char *buffer)
{
    ...
    if(!*loc)
    {
        ...
        if(!Glo.IP.addrcount)
        {
            write("No Discover Table entries exist.\r\n");
            return;
        }
        for(i = 0; i < Glo.IP.addrcount; i++)
        {
            addr = Glo.IP.discover[i];
            sprintf(message, "IP Address %d: %d.%d.%d.%d\r\n", i,
                   (uint8_t) (addr >> 24), (uint8_t) (addr >> 16),
    ...
    else if(strstr(loc, "find") == loc)
    {
        addr = Glo.IP.host;
        sprintf(message, "-netudp multicast -discover %d.%d.%d.%d t",
                (uint8_t) (addr >> 24), (uint8_t) (addr >> 16),
                (uint8_t) (addr >> 8), (uint8_t) addr);
        MSGParser_NETUDP(message, 0);
    }
    else if(*loc == 'c')
    {
        int i;
        for(i = 0; i < Glo.IP.addrcount; i++)
            Glo.IP.discover[i] = 0;
        Glo.IP.addrcount = 0;
        write("All Discover Table entries successfully removed.\r\n");
    }
    else if(isdigit(*loc))
    {
        addr = getAddr(loc);
        if(!addr)
        {
            ...
            {
                write("Invalid IP Address or Discover Table entry.\r\n");
                return;
            }
        }
        loc = nextword(loc);
        ...
        {
            addr = Glo.IP.host;
            sprintf(message, "-netudp multicast -discover %d.%d.%d.%d",
                    (uint8_t) (addr >> 24), (uint8_t) (addr >> 16),
                    (uint8_t) (addr >> 8), (uint8_t) addr);
            MSGParser_NETUDP(message, 0);
        }
...
{
   char *loc, *portloc = 0, udpbuf[UDPPACKET_SIZE];
   uint32_t saddr, gateKey;
   int Qnext, l, buflen;
   uint16_t port;
   loc = nextword(buffer);

   if(strstr(loc, "localhost") == loc)
       saddr = Glo.IP.localhost;
   else if(strstr(loc, "broadcast") == loc)
       saddr = Glo.IP.broadcast;
   else if(strstr(loc, "multicast") == loc)
       saddr = Glo.IP.multicast;
   else if(*loc == 'd')
   {
       char *entry;
       int i;
       ...
       if(i < 0 || i >= Glo.IP.addrcount)
       {
           sprintf(udpbuf, "No IP Address exists at Discover Table entry %d", i);
           write(udpbuf);
           return;
       }
       saddr = Glo.IP.discover[i];
   }
   else if(isdigit(*loc))
   {
       saddr = getAddr(loc);
       if(!saddr)
       {
           addPayload("-print Invalid IP Address!");
           return;
       }
       portloc = strchr(loc, ':');
   }
   ...
   if(!*loc)
   {
       addPayload("-print Invalid Payload");
       return;
   }
   if(loc > portloc && portloc)
   {
       ...
       if(!isdigit(*portloc))
           ...
           port = atoi(portloc);
       sprintf(udpbuf, "%d.%d.%d.%d:%d %s", (uint8_t) (saddr >> 24), (uint8_t) (saddr >> 16),
                  (uint8_t) (saddr >> 8), (uint8_t) saddr, port, loc);
   }
   else
       sprintf(udpbuf, "%d.%d.%d.%d %s", (uint8_t) (saddr >> 24), (uint8_t) (saddr >> 16),
               (uint8_t) (saddr >> 8), (uint8_t) saddr, loc);

   gateKey = GateSwi_enter(Glo.Bios.gateUDPOut);

   if(Glo.QUDPOut.write >= QLENGTH - 1)
       Qnext = 0;
   else
       Qnext = Glo.QUDPOut.write + 1;
   if(Qnext == Glo.QUDPOut.read)
   {
       addError("qoverflow");
       GateSwi_leave(Glo.Bios.gateUDPOut, gateKey);
       return;
   }
   strcpy(Glo.QUDPOut.buffer[Glo.QUDPOut.write], udpbuf);
   l = strlen(udpbuf) + 1;
   buflen = strlen(buffer) + 1;
   Glo.QUDPOut.bytecount[Glo.QUDPOut.write] = bytecount;
   if(bytecount > 0)
   {
       //memcpy(&dummy, Glo.ADCBuf.completedRx, bytecount);
       memcpy(&Glo.QUDPOut.buffer[Glo.QUDPOut.write][l], &buffer[buflen], bytecount);
   }
   Glo.QUDPOut.write = Qnext;
   Semaphore_post(Glo.Bios.semUDPOut);
   GateSwi_leave(Glo.Bios.gateUDPOut, gateKey);
}
...
void MSGParser_Stream(char *buffer)
{
    ...
    if(!*loc)
    {
        ...
        MSGParser_GPIO("-gpio 4 w 0");
        Glo.Devices.spiTransaction.txBuf = &Glo.AudioOut;
        if(Glo.Fs != 8000)
...
void MSGParser_Voice(char *buffer, uint32_t addr)
{
    ...
    if(choice != 0 && choice != 1)
        return;
    loc = nextword(loc);
    if(atoi(loc) != AUDIO_BLOCK_SIZE)
        return;
    loc = nextword(loc);
    loc++;
    if(choice)
        memcpy((void *) Glo.AudioRx[index].PongBuffRx, (void *) loc, 2 * AUDIO_BLOCK_SIZE);
    else
        memcpy((void *) Glo.AudioRx[index].PingBuffRx, (void *) loc, 2 * AUDIO_BLOCK_SIZE);
    ...
    Glo.AudioQ.index[Glo.AudioQ.write] = index;
