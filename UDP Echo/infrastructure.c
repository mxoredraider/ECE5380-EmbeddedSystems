...
extern GateSwi_Handle gateSwi0, gateSwi1, gateSwi2, gateSwi3, gateSwi4;
extern Semaphore_Handle semaphore0, semaphore1, semaphore2;
extern Swi_Handle swi0, swi1, swi2, swi3, swi4;
...
void addCall(uint32_t addr)
{
    ...
    for(i = 0; i < Glo.Call.callcount; i++)
    {
        ...
        {
            sprintf(message, "%d.%d.%d.%d is already on the call.\r\n",
    ...
    {
        Glo.Call.local = 0;
        MSGParser_Stream("-stream");
    }
    Glo.Call.list[Glo.Call.callcount] = addr;
    Glo.Call.callcount++;
    sprintf(message, "IP Address %d: %d.%d.%d.%d added to the call.\r\n", i,
           (uint8_t) (addr >> 24), (uint8_t) (addr >> 16),
           (uint8_t) (addr >> 8), (uint8_t) addr);
    write(message);
}
    ...
    for(i = 0; i < Glo.Call.callcount; i++)
    {
        if(!removed)
        {
            if(Glo.Call.list[i] == addr)
            {
                Glo.Call.list[i] = 0;
                removed = 1;
                sprintf(message, "%d.%d.%d.%d removed from the call.\r\n",
                        (uint8_t) (addr >> 24), (uint8_t) (addr >> 16),
                        (uint8_t) (addr >> 8), (uint8_t) addr);
                write(message);
            }
            ...
            Glo.Call.list[i - 1] = Glo.Call.list[i];
    }
    if(!removed)
    {
        sprintf(message, "%d.%d.%d.%d is not connected on the call.\r\n",
                (uint8_t) (addr >> 24), (uint8_t) (addr >> 16),
                (uint8_t) (addr >> 8), (uint8_t) addr);
        write(message);
    }
...
void addDiscover(uint32_t addr)
{
    ...
    {
        if(Glo.IP.discover[i] == addr)
        {
            sprintf(message, "%d.%d.%d.%d already exists in the Discover Table.\r\n",
                    (uint8_t) (addr >> 24), (uint8_t) (addr >> 16),
                    (uint8_t) (addr >> 8), (uint8_t) addr);
            write(message);
            return;
    ...
    Glo.IP.discover[Glo.IP.addrcount] = addr;
    Glo.IP.addrcount++;
    ...
    for(i = 0; i < Glo.IP.addrcount; i++)
    {
        if(!removed)
        {
            if(Glo.IP.discover[i] == addr)
            {
                Glo.IP.discover[i] = 0;
                removed = 1;
                sprintf(message, "%d.%d.%d.%d successfully removed from the Discover Table.\r\n",
                        (uint8_t) (addr >> 24), (uint8_t) (addr >> 16),
                        (uint8_t) (addr >> 8), (uint8_t) addr);
                write(message);
            }
            ...
            Glo.IP.discover[i - 1] = Glo.IP.discover[i];
    }
    if(!removed)
    {
        sprintf(message, "%d.%d.%d.%d does not exist in the Discover Table.\r\n",
                (uint8_t) (addr >> 24), (uint8_t) (addr >> 16),
                (uint8_t) (addr >> 8), (uint8_t) addr);
        write(message);
    }
...
void Device_init()
{
    ...
    Glo.IP.broadcast = 0xFFFFFFFF;  // 255.255.255.255
    Glo.IP.host = 0;
    Glo.IP.localhost = 0x7F000001;  // 127.0.0.1
    Glo.IP.multicast = 0xEFFFFF01;  // 239.255.255.1
    for(i = 0; i < DISCOVER_COUNT; i++)
    {   Glo.IP.discover[i] = 0; }

    /* Initialize IP Call */
    Glo.Call.callcount = 0;
    Glo.Call.local = 1;
...
void ClearAudioBuffers()
{
    ...
    for(i = 0; i < DISCOVER_COUNT; i++)
    {
        ...
        {
            Glo.AudioTx[i].PingBuffTx[j] = AUDIO_INIT;
            Glo.AudioTx[i].PongBuffTx[j] = AUDIO_INIT;
        }
        Glo.AudioTx[i].completedTx = 0;
        Glo.AudioTx[i].readindex = 0;
        Glo.AudioTx[i].state = 0;
    }
}
...
uint32_t getAddr(char *addrstr)
{
    ...
    {
        addr = addr << 8;
        addr = (addr & 0xFFFFFF00) + (atoi(addrstr) & 0x000000FF);
        if(i < 3)
        {
            while(*addrstr != '.' && *addrstr)
                addrstr++;
            if(!*addrstr)
                return 0;
            else
