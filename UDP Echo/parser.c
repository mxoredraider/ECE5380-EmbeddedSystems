...
void MSGParser(char *buffer)
{
    ...
    {   write("\xc"); }
    else if(strstr(bufflow, "-dial") == bufflow)
    {   MSGParser_Dial(bufflow);    }
    else if(strstr(bufflow, "-discover") == bufflow)
    {   MSGParser_Discover(bufflow);    }
    ...
    else if(strstr(bufflow, "-netudp") == bufflow)
    {   MSGParser_NETUDP(bufflow, 0);  }
